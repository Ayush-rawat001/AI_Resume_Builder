      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative w-full sm:w-64">
            ...
            <select 
              className="input-field pl-10 h-11 text-sm bg-white/50 border-stone-200"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {!searching && !results && history.length === 0 && (
            <div className="card p-12 flex flex-col items-center justify-center text-center border-dashed border-stone-200 bg-transparent">
                ...
                <div key={i} className="card p-5 hover:border-jade-500/40 transition-all group overflow-hidden relative">
                   ...
                   <div className="flex items-start justify-between gap-4">
                     <div className="flex-1 min-w-0">
                       <h3 className="font-bold text-stone-900 text-lg group-hover:text-orange-500 transition-colors truncate">{job.title}</h3>
                       <p className="text-sm text-stone-400 font-medium mb-4">{job.company} • {job.location}</p>
                       
                       <div className="flex flex-wrap gap-1.5 mb-5">
                         {job.matchingSkills.slice(0, 4).map(s => (
                           <span key={s} className="px-2 py-0.5 rounded-md bg-orange-50 text-orange-500 text-[10px] font-bold uppercase border border-orange-200">
                             {s}
                           </span>
                         ))}
                     ...
                     <div className="flex flex-col items-end gap-3">
                        <div className="flex flex-col items-center justify-center w-14 h-14 rounded-2xl bg-stone-50 border border-stone-200 shadow-inner">
                          <span className={`text-lg font-bold ${job.matchScore > 70 ? 'text-orange-500' : 'text-orange-500'}`}>{job.matchScore}%</span>
                          <span className="text-[8px] text-stone-400 font-bold uppercase tracking-tighter">Match</span>
                        </div>
                        <a 
                          href={job.url} 
                          target="_blank" 
                          rel="noopener noreferrer"
                          className="p-2 rounded-xl bg-stone-100/50 text-stone-600 hover:text-stone-900 hover:bg-orange-500 transition-all shadow-lg"
                        >
          ...
          {!searching && !results && history.length > 0 && (
             <div className="space-y-4">
                <h2 className="text-lg font-bold text-stone-900 mb-4">Match History</h2>
                {history.slice(0, 10).map((h) => (
                  <div key={h.matchId} className="card p-4 border-stone-100 bg-white/40 flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-stone-50 flex items-center justify-center text-stone-400">
                        <CheckCircle2 size={20} className={h.matchScore > 70 ? 'text-orange-500' : 'text-orange-500'} />
                      </div>
                      <div>
                        <p className="text-sm font-bold text-stone-900 truncate max-w-[200px]">{h.jobTitle}</p>
                        <p className="text-[11px] text-stone-400">{h.companyName} • {new Date(h.matchedAt).toLocaleDateString()}</p>
                      </div>
                    </div>
        ...
        <div className="space-y-6">
          <div className="card p-6 bg-gradient-to-br from-orange-500/10 to-transparent border-orange-200">
            ...
            <ul className="space-y-4">
              <li className="flex gap-3">
                ...
                <p className="text-xs text-stone-600 leading-relaxed">Add at least <span className="text-stone-900 font-bold">10 skills</span> to your resume for more accurate matching.</p>
              </li>
              <li className="flex gap-3">
                <CheckCircle2 size={14} className="text-orange-500 mt-0.5 shrink-0" />
                <p className="text-xs text-stone-600 leading-relaxed">Use standard industry terms like <span className="text-stone-900 font-bold">"React"</span> instead of "Frontend coding".</p>
              </li>
              <li className="flex gap-3">
                <CheckCircle2 size={14} className="text-orange-500 mt-0.5 shrink-0" />
                <p className="text-xs text-stone-600 leading-relaxed">Our AI prioritizes remote-first companies from the <span className="text-orange-500 font-bold italic underline">Himalayas</span> network.</p>
              </li>
            </ul>
          </div>

          <div className="card p-6">
             <h3 className="font-bold text-stone-900 mb-4">Your Search Stats</h3>
             <div className="grid grid-cols-2 gap-4">
                <div className="bg-stone-50/50 p-3 rounded-xl border border-stone-200">
                   <p className="text-[10px] text-stone-400 uppercase tracking-widest mb-1">Total Scans</p>
                   <p className="text-2xl font-bold text-stone-900">{history.length}</p>
                </div>
                <div className="bg-stone-50/50 p-3 rounded-xl border border-stone-200">
                   <p className="text-[10px] text-stone-400 uppercase tracking-widest mb-1">Bookmarks</p>
                   <p className="text-2xl font-bold text-orange-500">{history.filter(h => h.isBookmarked).length}</p>
                </div>
             </div>
          </div>

          <div className="p-4 rounded-2xl bg-white/60 border border-stone-100 flex items-start gap-4">
             <AlertCircle size={18} className="text-stone-400 shrink-0 mt-0.5" />
